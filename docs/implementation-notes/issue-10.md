...
- End-to-end sanity on the real emulator (`--attack --seed 42` over `tests/fixtures`): harness scored 3366 queries; DGA recall 83.1 % / precision 88.8 %; benign elimination rate 66.7 % (only 6 benign fixtures exist in that tiny window); tunnel/beaconing/typosquat episodes compress into a ~2 s replay window where the corresponding thresholded signals (repetition, >= 5 s intervals, entropy >= 3.5) cannot fire — the harness surfaces this as low recall, which is the expected observable for the fixture scale.