...
| P6 | Kafka: 1 topic `dns.telemetry.v1`, 3 partitions keyed by `client_ip`, ~1 h retention; ClickHouse: `dns_events_raw` (MergeTree by day, `ORDER BY (ts, client_ip)`) + `site_qoe_minute` (`ORDER BY (site, ts)`) |
