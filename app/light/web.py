...
:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#070b14;color:#e8eefc;font:14px Inter,ui-sans-serif,system-ui,-apple-system,sans-serif}.wrap{max-width:1180px;margin:auto;padding:36px 22px}.top{display:flex;justify-content:space-between;align-items:end;margin-bottom:26px}.brand{font-size:26px;font-weight:750;letter-spacing:.08em}.sub{color:#7f8da8;margin-top:7px}.live{font-size:12px;color:#9fb4d9}.dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:#6ee7b7;margin-right:7px}.cards{display:grid;grid-template-columns:repeat(5,1fr);gap:12px}.card,.panel{border:1px solid #172238;background:linear-gradient(180deg,#0d1422,#0a101c);border-radius:14px}.card{padding:18px}.label{color:#73819b;font-size:11px;text-transform:uppercase;letter-spacing:.12em}.num{font-size:26px;margin-top:9px;font-variant-numeric:tabular-nums}.grid{display:grid;grid-template-columns:1.3fr .7fr;gap:12px;margin-top:12px}.panel{padding:18px;min-height:310px}h2{font-size:13px;margin:0 0 16px;letter-spacing:.04em}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:10px 7px;border-bottom:1px solid #151f31;font-size:12px}th{color:#65738d;font-weight:500}.pill{padding:4px 8px;border:1px solid #293854;border-radius:999px}.bar{height:7px;background:#182238;border-radius:9px;overflow:hidden;margin:7px 0 14px}.fill{height:100%;background:#8ba7d9}.qoe{display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #151f31}.score{font-size:18px;font-variant-numeric:tabular-nums}.evidence{color:#94a4c0;font-size:11px;margin-top:5px;max-width:410px;white-space:normal}@media(max-width:900px){.cards{grid-template-columns:1fr 1fr}.grid{grid-template-columns:1fr}.top{align-items:start;flex-direction:column;gap:10px}}
</style></head><body><div class="wrap"><div class="top"><div><div class="brand">SHIELD</div><div class="sub">Private DNS intelligence · local runtime</div></div><div class="live"><span class="dot"></span>QVAC LOCAL · CLOUD INFERENCE DISABLED</div></div><div class="cards"><div class="card"><div class="label">DNS queries</div><div class="num" id="events">—</div></div><div class="card"><div class="label">QVAC candidates</div><div class="num" id="candidates">—</div></div><div class="card"><div class="label">Filter reduction</div><div class="num" id="reduction">—</div></div><div class="card"><div class="label">Alerts</div><div class="num" id="alerts">—</div></div><div class="card"><div class="label">Avg QVAC latency</div><div class="num" id="lat">—</div></div></div><div class="grid"><div class="panel"><h2>Recent detections · deterministic evidence + local AI verdict</h2><table><thead><tr><th>Domain / evidence</th><th>Verdict</th><th>Confidence</th><th>Client</th></tr></thead><tbody id="rows"></tbody></table></div><div><div class="panel"><h2>Threat distribution</h2><div id="verdicts"></div></div><div class="panel" style="margin-top:12px;min-height:200px"><h2>Network experience</h2><div id="qoe"></div></div></div></div></div><script>
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const evidence=x=>Object.keys(x.signals||{}).join(' · ')||'deterministic escalation';async function refresh(){let d=await fetch('/api/snapshot').then(r=>r.json());events.textContent=d.events.toLocaleString();candidates.textContent=d.candidates.toLocaleString();reduction.textContent=d.filter_reduction_percent.toFixed(2)+'%';alerts.textContent=d.alerts.toLocaleString();lat.textContent=d.avg_qvac_ms.toFixed(0)+' ms';rows.innerHTML=d.recent_alerts.map(x=>`<tr><td>${esc(x.qname)}<div class="evidence">Signals: ${esc(evidence(x))}<br>QVAC: ${esc(x.reasoning||'—')}</div></td><td><span class="pill">${esc(x.verdict)}</span></td><td>${Math.round(100*x.confidence)}%</td><td>${esc(x.client_ip)}</td></tr>`).join('')||'<tr><td colspan=4>No alerts yet</td></tr>';let mx=Math.max(1,...d.verdicts.map(x=>x.count));verdicts.innerHTML=d.verdicts.map(x=>`<div>${esc(x.verdict)} · ${x.count}<div class="bar"><div class="fill" style="width:${100*x.count/mx}%"></div></div></div>`).join('')||'No detections yet';qoe.innerHTML=d.qoe.slice(0,8).map(x=>`<div class="qoe"><div>${esc(x.site)}<div class="sub">${esc(x.label)} · p95 ${Math.round(x.p95_latency_ms)} ms</div></div><div class="score">${x.score}</div></div>`).join('')||'No QoE windows yet'}refresh();setInterval(refresh,2000)</script></body></html>'''
